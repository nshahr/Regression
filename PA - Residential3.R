...
GGDFrame=data.frame(FC[1:119], PA_Res$Avg_Con[1:119], x=PA_Res$Month[1:119])
MGGDFrame=melt(GGDFrame, id="x")

ggplot(MGGDFrame, aes(x,value, colour = variable, xlab= "Months")) + geom_line(size = 1) + scale_color_manual(values = c("red","black")) + geom_point(shape = 16, size = 2)
head(MGGDFrame)

#to overcome the shift:

GGDFrame=data.frame(Estimated_Consumption = FC[1:118], Actual_Consumption = PA_Res$Avg_Con[2:119], x=PA_Res$Month[2:119])
MGGDFrame=melt(GGDFrame, id="x")
